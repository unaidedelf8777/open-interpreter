...
from .container_utils import build_docker_images
...
class Interpreter:
  ...
  def __init__(self, session_id=None, contain=True):
    self.messages = []
    ...
    if session_id is not None:
      ...
      self.session_id = "sid-" + str(uuid.uuid4())
    ...
    if not self.local:
      ...
      with open(os.path.join(here, 'llama_sys_message.txt'), 'r') as f:
        self.system_message = f.read().strip()
      ...
      if chunk["choices"][0]["finish_reason"] or llama_function_call_finished:
        if chunk["choices"][
          ...
          if language not in self.code_interpreters:
            self.code_interpreters[language] = CodeInterpreter(language, self.debug_mode, self.contain, self.session_id)
